...
.macro fpTest(value, f) {
    ...
    shiftLeft16bit(f, two)

    copy16bit(halfOrig, half)
    shiftLeft16bit(f-8, half) // Because halfOrig already in Q8.8 - we only shifting f-8 bits

    mulFixedPoint(value, two, valueTmp, f)
    copy16bit(valueTmp, value)
    mulFixedPoint(value, half, valueTmp, f)
    copy16bit(valueTmp, value)
exit:
}
...
*=$2000 "Data"
    ...
    halfOrig: .word 128 // Already in Q8.8
    half: .word 0
